...
export function initSelfie(cfg: AppConfig): void {
  config = cfg;
  referencePath = path.join(cfg.statePath, "selfie", "reference.png");
  statePath = path.join(cfg.statePath, "selfie", "selfie-state.json");
  generatedDir = path.join(cfg.statePath, "selfie", "generated");
  ...
  try {
    referenceBase64 = fs.readFileSync(referencePath).toString("base64");
...
export function isVisualIdentityEnabled(): boolean {
  if (!config?.falApiKey) return false;
...
export async function generateSelfie(
  ...
  if (!isVisualIdentityEnabled() || !config || !referenceBase64) return null;
  ...
  try {
    ...
    const prompt = await generateSelfiePrompt(trigger, conversationContext);
    if (!prompt) {
    ...
    while (attempts < MAX_RETRIES) {
      ...
      // Call fal.ai FLUX
      const imageUrl = await callFalApi(prompt);
      ...
      const quality = await checkImageQuality(downloaded);
      if (quality.pass) {
    ...
    // Generate caption
    const caption = generateCaption(trigger);
...
export function detectSelfieTrigger(text: string): SelfieTrigger | null {
  if (!isVisualIdentityEnabled()) return null;
...
export function shouldSendSticker(): boolean {
  if (!isVisualIdentityEnabled()) return false;
  const state = loadState();
...
export function recordSelfieContext(trigger: SelfieTrigger, emotionalContext: string): void {
  const state = loadState() as SelfieState & { recentContexts?: RecentSelfieContext[] };
  ...
  saveState(state as SelfieState);
}
...
async function generateSelfiePrompt(trigger: SelfieTrigger, conversationContext?: string): Promise<string | null> {
...
async function callFalApi(prompt: string): Promise<string | null> {
  ...
  if (hasLora) {
    ...
    payload.image_url = `data:image/png;base64,${referenceBase64}`;
...
function generateCaption(trigger: SelfieTrigger): string {
...
function cleanupOldImages(state: SelfieState): void {
...
function loadState(): SelfieState {
  ...
  const loaded = readJsonSafe<SelfieState>(statePath, defaults);
...
function saveState(state: SelfieState): void {
...
// ── Proactive selfie — dynamic scene generation ─────────────────────
...
export async function maybeProactiveSelfie(context: {
  ...
  try {
    ...
    const selfie = await generateSelfie(trigger, parsed.scene);
    return selfie !== null;
  } catch (err) {
...
// ── Exports for sticker script ───────────────────────────────────────

export { callFalApi, loadState, saveState };
