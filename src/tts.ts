...
export function initTTS(cfg: AppConfig): void {
  config = cfg;
  ttsDir = path.join(cfg.statePath, "tts");
  stateFilePath = path.join(ttsDir, "state.json");
...
export function isTTSEnabled(): boolean {
  return !!(config?.fishAudioApiKey && config?.fishAudioVoiceId);
}
...
export async function generateVoice(
  ...
  if (!isTTSEnabled() || !config) return null;
  ...
  try {
    ...
    const opusBuffer = await callFishAudio(ttsText, voiceParams);
    if (!opusBuffer) return null;

    // Wrap opus in OGG container for Telegram voice messages
    const oggBuffer = await wrapOpusInOgg(opusBuffer);
...
async function callFishAudio(
  ...
  if (!config?.fishAudioApiKey || !config?.fishAudioVoiceId) return null;
 ...
 */
function wrapOpusInOgg(opusBuffer: Buffer): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const tmpIn = path.join(ttsDir, `_tmp_${Date.now()}.opus`);
    const tmpOut = path.join(ttsDir, `_tmp_${Date.now()}.ogg`);
...
function loadState(): TTSState {
  return readJsonSafe<TTSState>(stateFilePath, {
    dailyDate: "",
    dailyCount: 0,
    lastTTSAt: 0,
  });
...
function saveState(state: TTSState): void {
