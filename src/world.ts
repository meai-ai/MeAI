...
export function initWorld(config: { statePath: string }): void {
...
function getLocationDetail(location: string): string {
...
/**
 * Fetch current weather for San Francisco from Open-Meteo (free, no API key).
 */
export async function fetchWeather(): Promise<WeatherSnapshot | null> {
  ...
  try {
    ...
    const snapshot: WeatherSnapshot = {
      ...
      condition: getWmoDescription(weatherCode),
...
/**
 * Check if it's currently daylight in SF using real sunrise/sunset from Open-Meteo.
 * Returns a descriptive string, or null if weather data isn't available.
 */
export function getDaylightStatus(): string | null {
  if (!weatherCache?.sunrise || !weatherCache?.sunset) return null;
  ...
  const [sunriseH, sunriseM] = weatherCache.sunrise.split(":").map(Number);
  const [sunsetH, sunsetM] = weatherCache.sunset.split(":").map(Number);
  const sunriseMinutes = sunriseH * 60 + sunriseM;
  const sunsetMinutes = sunsetH * 60 + sunsetM;

  if (nowMinutes < sunriseMinutes) return s().daylight.dark.replace("{time}", weatherCache.sunrise);
  if (nowMinutes >= sunsetMinutes) return s().daylight.night.replace("{time}", weatherCache.sunset);
  // Within 30 min of sunset
  if (sunsetMinutes - nowMinutes <= 30) return s().daylight.dusk.replace("{time}", weatherCache.sunset);
  return s().daylight.dawn.replace("{time}", weatherCache.sunset);
}

/** Format weather for system prompt */
export function formatWeather(w: WeatherSnapshot | null): string {
  if (!w) return "";
...
let marketCache: { text: string; fetchedAt: number } | null = null;
const MARKET_CACHE_TTL = 30 * 60 * 1000;

/**
 * Fetch live market data for today's watchlist.
 * The watchlist comes from the LLM-generated daily schedule.
 */
export async function fetchMarketSnapshot(): Promise<string> {
  if (marketCache && Date.now() - marketCache.fetchedAt < MARKET_CACHE_TTL) {
    return marketCache.text;
  }
  ...
  const watchlist = loadCachedWatchlist();
  ...
  try {
    ...
    marketCache = { text, fetchedAt: Date.now() };
    return text;
  } catch {
    return "";
  }
...
function loadCachedWatchlist(): WatchlistItem[] {
...
// ── Daily Schedule (LLM-generated) ───────────────────────────────────

/** File-based cache so schedule survives restarts within the same day */
function getScheduleCachePath(): string {
  return path.join(dataPath, "daily-schedule.json");
}

function loadCachedSchedule(): DailySchedule | null {
  if (!dataPath) return null;
  const p = getScheduleCachePath();
...
function saveCachedSchedule(schedule: DailySchedule): void {
  if (!dataPath) return;
  fs.writeFileSync(getScheduleCachePath(), JSON.stringify(schedule, null, 2) + "\n");
}
 ...
 */
async function ensureDailySchedule(): Promise<DailySchedule> {
  const now = new Date();
  const userTime = new Date(now.toLocaleString("en-US", { timeZone: getUserTZ() }));
  const todayStr = `${userTime.getFullYear()}-${String(userTime.getMonth() + 1).padStart(2, "0")}-${String(userTime.getDate()).padStart(2, "0")}`;
...
function defaultBlocks(isWorkDay: boolean): TimeBlock[] {
  if (isWorkDay) return [
...
async function generateSchedule(ctx: {
  ...
  const weatherContext = ctx.weather
    ? `\nToday's weather: ${formatWeather(ctx.weather)}\n`
    : "";
  ...
  if (!schedulePrompt) {
    ...
    return {
      ...
      blocks: defaultBlocks(ctx.isWorkDay), dayOff: "",
      watchlist: [],
  ...
  try {
    ...
    if (parsed) {
      ...
      return {
        ...
        blocks: validBlocks.length > 0 ? validBlocks : defaultBlocks(ctx.isWorkDay),
  ...
  return {
    date: ctx.todayStr,
    isWorkDay: ctx.isWorkDay,
    blocks: defaultBlocks(ctx.isWorkDay),
    dayOff: "",
    watchlist: [],
    sleep: { bedtime: "23:00", wakeTime: "7:00", quality: 7, note: "normal" },
    outfit: "",
...
/**
 * Get the character's current life context — what she's doing right now.
 * Covers the full day, not just work hours.
 */
export async function getWorkContext(now: Date = new Date()): Promise<{
  ...
  if (!schedule.isWorkDay && !currentBlock) {
    return {
      ...
      busy: false,
      fullSchedule: formatFullDaySchedule(schedule, hour),
      isWorkDay: false,
      currentBlock: null,
      nextBlock,
      location: "home",
...
/**
 * Get sleep data for body state calculation.
 */
export async function getSleepData(): Promise<SleepLog> {
  const schedule = await ensureDailySchedule();
  return schedule.sleep;
}

/**
 * Get today's pet moments.
 */
export async function getPetMoments(): Promise<string[]> {
  const schedule = await ensureDailySchedule();
 ...
 * Get today's outfit.
 */
export async function getOutfit(): Promise<string> {
  const schedule = await ensureDailySchedule();
  return schedule.outfit;
}

/**
 * Get today's meal blocks for body state (hunger tracking).
 */
export async function getMealBlocks(): Promise<TimeBlock[]> {
  const schedule = await ensureDailySchedule();
  return schedule.blocks.filter(b => b.category === "meal");
}

/**
 * Get a rich time/space context snapshot.
 * Now uses the actual schedule to know exactly where she is.
 */
export async function getTimeSpaceContext(now: Date = new Date()): Promise<string> {
  const userTime = new Date(now.toLocaleString("en-US", { timeZone: getUserTZ() }));
  const hour = userTime.getHours();
  ...
  const locationDetail = getLocationDetail(work.location);

  // Weather
  const weather = await fetchWeather();
  const weatherStr = weather ? formatWeather(weather) : "";
...
/**
 * Calculate reply delay based on what the character is currently doing.
 * She doesn't always see messages instantly — sleeping, showering,
 * exercising, focused work, or socializing all cause natural delays.
 */
export async function calculateReplyDelay(now: Date = new Date()): Promise<{
  delayMs: number;
  reason: string;
}> {
  const { currentBlock } = await getWorkContext(now);
  const userTime = new Date(now.toLocaleString("en-US", { timeZone: getUserTZ() }));
  const hour = userTime.getHours();
...
function formatPreviousDay(prev: DailySchedule): string {
  const parts: string[] = [];
...
function formatFullDaySchedule(schedule: DailySchedule, currentHour?: number): string {
...
function loadFile(...segments: string[]): string {
  if (!dataPath) return "";
  const p = path.join(dataPath, ...segments);
  if (!fs.existsSync(p)) return "";
  return fs.readFileSync(p, "utf-8").trim();
}

function loadRecentMemories(): string {
  try {
