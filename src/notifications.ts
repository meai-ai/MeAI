...
export function initNotifications(statePath: string): void {
  dataPath = statePath;
}
...
function getStatePath(): string {
  return path.join(dataPath, "notification-state.json");
}
...
function loadState(): NotificationState {
  ...
  stateCache = readJsonSafe<NotificationState>(p, defaultState());
  return stateCache;
}
...
function addNotification(state: NotificationState, notif: Omit<Notification, "id" | "timestamp" | "read">): void {
...
async function checkYouTube(state: NotificationState): Promise<number> {
  let count = 0;
  try {
...
async function checkPodcasts(state: NotificationState): Promise<number> {
  let count = 0;
  try {
...
async function checkPriceAlerts(state: NotificationState): Promise<number> {
  let count = 0;
...
async function checkWeather(state: NotificationState): Promise<number> {
...
async function checkCalendar(state: NotificationState): Promise<number> {
  let count = 0;
  ...
  return [
    {
      ...
      execute: async (input: Record<string, unknown>): Promise<string> => {
        ...
        if (type === "youtube") {
          ...
          const id = addPriceAlert(ticker, stockName || ticker, condition, threshold, (input.recurring as boolean) ?? false);
          ...
          const id = addWeatherAlert(cond as "rain" | "temperature_drop" | "temperature_rise" | "any_change", threshold);
    ...
    },
    {
      ...
      execute: async (input: Record<string, unknown>): Promise<string> => {
        ...
        if (type === "youtube") {
          ...
          ok = removePriceAlert(id);
          target = `price_alert ${id}`;
        } else if (type === "weather_alert") {
          const id = input.id as string;
          if (!id) return "alert id required";
          ok = removeWeatherAlert(id);
    ...
    },
    {
      ...
      execute: async (): Promise<string> => {
        const rawList = listSubscriptions();
