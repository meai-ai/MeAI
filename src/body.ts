...
export class BodyEngine {
  ...
  calculatePeriodStatus(now: Date): PeriodStatus {
    ...
    if (daysSinceStart >= health.cycleLength) {
      ...
      health.cycleLength = 28 + variation;
      this.saveHealthState(health);
    }
    const symptoms: string[] = [];
    const extPhase = getCyclePhase(dayOfCycle, health.cycleLength, health.periodLength);
    const phase: PeriodPhase = extPhase as PeriodPhase;
  ...
  rollDailySickness(context: {
    ...
    if (Math.random() < sickChance) {
      ...
      health.sickSeverity = 4 + Math.floor(Math.random() * 4);
  ...
  calculateSickStatus(): SickStatus {
    const health = this.loadHealthState();
    if (health.sickType === "none") {
  ...
  async getBodyState(now: Date = new Date()): Promise<BodyState> {
    return getBodyState(now);
  }

  getCurrentPeriodPhase(): PeriodPhase {
    const now = new Date();
    const userTime = new Date(now.toLocaleString("en-US", { timeZone: getUserTZ() }));
    return this.calculatePeriodStatus(userTime).phase;
  }

  getCurrentSickStatus(): SickStatus {
    return this.calculateSickStatus();
  }

  getCycleMoodModifiers(): CycleMoodModifiers {
    return getCycleMoodModifiers();
  }
}

// ── Singleton ──────────────────────────────────────────────────────────

let _singleton: BodyEngine | null = null;
let dataPath = "";

export function initBody(statePath: string): BodyEngine {
  dataPath = statePath;
  _singleton = new BodyEngine(statePath);
  return _singleton;
}

// ── Backward-compat singleton wrappers ──────────────────────────────

/** @deprecated Use BodyEngine class directly */
...
export function getCycleMoodModifiers(): CycleMoodModifiers {
  ...
  const now = new Date();
  const userTime = new Date(now.toLocaleString("en-US", { timeZone: getUserTZ() }));
  const status = _singleton!.calculatePeriodStatus(userTime);
...
// ── Core Logic (kept at module level — uses world.ts cross-deps) ─────
 ...
 *
 * Kept at module level because it has cross-module dependencies on world.ts
 * (getSleepData, getMealBlocks, getWorkContext, fetchWeather).
 * The BodyEngine.getBodyState() method delegates here.
 */
export async function getBodyState(now: Date = new Date()): Promise<BodyState> {
  ...
  // ── Period status ──
  const periodStatus = _singleton!.calculatePeriodStatus(userTime);
  ...
  else if (periodStatus.phase === "follicular") fatigue -= 0.5;
  ...
  if (exercisedToday && exerciseDetail) {
    const profile = DEFAULT_BODY_CONFIG.exerciseProfiles[exerciseDetail]
      ?? DEFAULT_BODY_CONFIG.exerciseProfiles[exerciseKeywords[exerciseKeywords.length - 1]]
      ?? Object.values(DEFAULT_BODY_CONFIG.exerciseProfiles)[0];
    if (profile) {
