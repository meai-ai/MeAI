...
export class FriendsEngine {
  ...
  private getStatePath(): string {
    return path.join(this.dataPath, "friend-state.json");
  }

  loadFriendState(): FriendState {
    if (!this.dataPath) return defaultFriendState();
  ...
  saveFriendState(state: FriendState): void {
  ...
  recordFriendMeeting(
    ...
    const state = this.loadFriendState();
    ...
    this.saveFriendState(state);
  }

  /** Set next planned meetup */
  planFriendMeeting(friendKey: string, plan: string): void {
    const state = this.loadFriendState();
    const friend = state.friends[friendKey];
    if (!friend) return;
...
let _singleton: FriendsEngine | null = null;

export function initFriends(statePath: string): FriendsEngine {
  _singleton = new FriendsEngine(statePath);
  return _singleton;
}

function _get(): FriendsEngine {
  if (!_singleton) throw new Error("initFriends() not called");
  return _singleton;
}

export function loadFriendState(): FriendState { return _get().loadFriendState(); }
export function saveFriendState(state: FriendState): void { _get().saveFriendState(state); }
export function recordFriendMeeting(friendKey: string, activity: string, topics?: string[]): void { _get().recordFriendMeeting(friendKey, activity, topics); }
export function planFriendMeeting(friendKey: string, plan: string): void { _get().planFriendMeeting(friendKey, plan); }
export function updateTieStrength(): void { _get().updateTieStrength(); }
export function recordInteraction(friendKey: string, type: string, initiated: boolean): void { _get().recordInteraction(friendKey, type, initiated); }
export function getSocialContext(): SocialContext { return _get().getSocialContext(); }
export function formatFriendContext(): string { return _get().formatFriendContext(); }
export function formatSocialSummary(): string { return _get().formatSocialSummary(); }
